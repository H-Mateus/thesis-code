...
setwd("../../thesis-code/500_patient_modelling")
# load data - Note: use of .rda here is to maintain df column data types
load('data/df_locf_fist_blood_complete.rda')
#load('../../mateus/500_patient/data/df_locf_first_blooc_complete_2019-10-21.rda')

# create some fake data for sharing
#library(fakeR)
# convert from tibble to dataframe
#fake_df <- as.data.frame(df_locf_first_blood_cc)
# create a simulated dataset - note: some of the factors might be given invalid values like 2 or -1 
#df_locf_first_blood_cc <- as.data.frame(simulate_dataset(fake_df, digits = 3, stealth.level = 3))
#save(df_locf_first_blood_cc, file = "../../thesis-code/500_patient_modelling/data/df_locf_fist_blood_complete.rda")
...
# get dataframe with bloods to remove
...
# NOTE: everything after this line is just analysis of the models generated by the above line
...
# filter to linear models without elastic net
...
# variables that have importance of 0, and 2nd a list of plots of the variable
# importance
...
# get bloxplot of bloods by improver
map(remaining_bloods, ~ gg_boxplot(glm_models_long[[1]], .x))
# get boxplot of all variables
map(all_vars, ~ gg_boxplot(glm_models_long[[1]], .x))
